...
DB_NAME="${db_name}"
DB_USER="${db_user}" 
DB_PASS=$(aws ssm get-parameter --name "/database/password" --query "Parameter.Value" --output text --with-decryption)
EFS_ID="${efs_id}"
LB_DNS="${lb_dns}"
...
define('WP_SITEURL', 'http://$APP_DOMAIN');" /var/www/html/wp-config.php
  ...
  # ================================
  # Reuse existing deployment - just update config
  # ================================
...
# ================================
cat << 'SCRIPT' > /var/www/html/wp_config_check.sh
#!/bin/bash
# Check and update DB password from SSM Parameter Store
echo "Checking for database password update..."
# Get current password from SSM
SSM_PASSWORD=$(aws ssm get-parameter --name "/database/password" --with-decryption --query "Parameter.Value" --output text --region us-east-1 2>/dev/null)
# Get current password from wp-config
CONFIG_PASSWORD=$(grep "DB_PASSWORD" /var/www/html/wp-config.php | awk -F"'" '{print $4}')
if [ -z "$SSM_PASSWORD" ]; then
    echo "ERROR: Could not retrieve password from SSM"
    exit 1
fi

if [ "$CONFIG_PASSWORD" == "$SSM_PASSWORD" ]; then
    echo "Password is current - no update needed"
else
    echo "Updating password in wp-config.php"
    sed -i "s/$CONFIG_PASSWORD/$SSM_PASSWORD/" /var/www/html/wp-config.php
...
# Set up cron to check every minute
(crontab -l 2>/dev/null; echo "* * * * * /var/www/html/wp_config_check.sh >> /var/log/wp_config_check.log 2>&1") | crontab -
